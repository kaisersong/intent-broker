...
## Xiaok Desktop v1.4.20 Integration Notes

- Intent Broker remains the event-first coordination layer for Xiaok Desktop v1.4.20, KSwarm project handoffs, scheduled loop dispatch, and local agent runtime adapters.
...
- Xiaok Desktop v1.4.20 reads task-completion loop results from Desktop evidence records and task snapshots after the broker-delivered work finishes. This adds user-facing result visibility without changing broker delivery semantics.
- No broker protocol migration is required for the Xiaok v1.4.20 README baseline; existing inbox delivery, event replay, hook installation, and Unix socket fallback semantics remain the active integration contract. The packaged broker baseline remains `0.3.8`.
