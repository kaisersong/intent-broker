...
## Xiaok Desktop v1.4.22 集成说明

- Intent Broker 仍是 Xiaok Desktop v1.4.22、KSwarm 项目 handoff、定时 loop 派发和本地 agent runtime adapter 使用的 event-first 协作层。
...
- Xiaok Desktop v1.4.22 会在 broker 投递的工作完成后，从 Desktop evidence record 和 task snapshot 读取 task-completion loop 结果；同一版本在本地修复随包 Computer Use 激活，不会把 CuaDriver 生命周期事件交给 Broker 路由。
...
- 本次 Xiaok v1.4.22 README 基线不要求 broker 协议迁移；现有 inbox delivery、event replay、hook 安装和 Unix socket fallback 语义仍是当前集成合同。随包 broker 基线仍是 `0.3.8`。
