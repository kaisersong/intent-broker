...
- Codex 的 auto-dispatch resume 现在不会重绑主会话 sidecar，因此在 `@all` 扇出或队列自动起工时不会制造假的上线 / 离线抖动
...
- 这些 auto-dispatch 的子 resume 现在会直接短路 Codex broker hooks，而不是重新拉起 keeper / realtime bridge sidecar，所以主会话在自动起工过程中不会再出现噪声式的离线 / 上线 presence 抖动。
...
如果 broker 还没启动，CLI 会直接给出可读提示，而不是抛一屏 Node stack trace，例如：
...
如果 broker 离线，这些本地桥接命令现在也会给出同样的友好提示，而不是直接抛 Node 堆栈：
