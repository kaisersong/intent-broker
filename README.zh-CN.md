...
## Xiaok Desktop v1.4.11 集成说明

- Intent Broker 仍是 Xiaok Desktop v1.4.11、KSwarm 项目 handoff、定时 loop 派发和本地 agent runtime adapter 使用的 event-first 协作层。
...
- 本次 Xiaok v1.4.11 README 基线不要求 broker 协议迁移；现有 inbox delivery、event replay、hook 安装和 Unix socket fallback 语义仍是当前集成合同。随包 broker 基线仍是 `0.3.8`。
