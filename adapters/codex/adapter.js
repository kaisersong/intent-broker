/**
 * Codex adapter for Intent Broker
 * Connects Codex to the Intent Broker for multi-agent collaboration
 */
import WebSocket from 'ws';

export class CodexAdapter {
  constructor({ brokerUrl, participantId, roles = ['coder', 'reviewer'], capabilities = [] }) {
  ...
  async connect() {
    ...
    console.log(`✓ Codex registered as ${this.participantId}`);
    ...
    // Update presence
    await this.updatePresence('online', { version: 'codex-1.0' });
  ...
  async handleIntent(event) {
    console.log(`[Codex] Received intent: ${event.kind} (${event.intentId})`);
    ...
    if (handler) {
      try {
        ...
        console.error(`[Codex] Error handling ${event.kind}:`, error.message);
      }
    }
  }
