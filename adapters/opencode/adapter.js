/**
 * OpenCode adapter for Intent Broker
 * Connects OpenCode to the Intent Broker for multi-agent collaboration
 */
...
export class OpenCodeAdapter {
  ...
  async connect() {
    ...
    console.log(`✓ OpenCode registered as ${this.participantId}`);
    ...
    // Update presence
    await this.updatePresence('online', { version: 'opencode-1.0' });
  ...
  async handleIntent(event) {
    console.log(`[OpenCode] Received intent: ${event.kind} (${event.intentId})`);
    ...
    if (handler) {
      try {
        ...
        console.error(`[OpenCode] Error handling ${event.kind}:`, error.message);
      }
    }
  }
